...
void main() {
  group('storage keys', () {
    ...
    test(
      'the second segment is the owner, which is what the policy checks',
      () {
  ...
  group('what the vault accepts', () {
    ...
    test('a photo of a paper certificate is fine', () {
      ...
      expect(
        UploadRules.reject(mimeType: 'image/jpeg', sizeBytes: 2 * 1024 * 1024),
        isNull,
      );
    });
    ...
    test('an unsupported type explains what is accepted instead', () {
      ...
      expect(
        message,
        isNot(contains('application/zip')),
    ...
    test('exactly at the limit is allowed', () {
      expect(
        UploadRules.reject(
          mimeType: 'application/pdf',
          sizeBytes: UploadRules.maxBytes,
        ),
        isNull,
      );
      expect(
