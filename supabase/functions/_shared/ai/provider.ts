...
export class MockProvider implements LLMProvider {
  ...
  async complete(request: CompletionRequest): Promise<CompletionResult> {
    const data = mockFixture(request.feature, request.user);
    return {
      data,
      provider: this.name,
      model: 'mock-1',
  ...
  switch (feature) {
    ...
    case 'parse_cv':
      return {
        ...
        summary: 'Final-year student with two personal projects and one internship.',
        skills: ['Python', 'SQL', 'Git'],
        education: [{ degree: 'BSc Computer Science', institution: 'University', year: 2027 }],
        experience: [{ title: 'Intern', organisation: 'A company', months: 3 }],
        projects: [{ title: 'A project', summary: 'Something built for a course.' }],
        quality_score: 62,
    ...
    case 'interview_questions':
      return {
        questions: [
          { question: 'Tell me about a project you are proud of.', category: 'behavioural' },
          { question: 'Describe a bug you found difficult to fix.', category: 'technical' },
          { question: 'How do you decide what to work on first?', category: 'behavioural' },
          { question: 'What would you like to be better at?', category: 'behavioural' },
