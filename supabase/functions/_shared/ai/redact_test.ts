...
Deno.test('email addresses never reach the model', () => {
  const { text, found } = redact('Contact me at rafiq.hossain@du.ac.bd for details.');
...
Deno.test('a CV with everything in it comes out clean', () => {
  ...
  assertClean(text);
  assert(text.includes('Final-year computer science student'),
...
Deno.test('redaction leaves ordinary text alone', () => {
  const input = 'Three years of Python and SQL, plus a 2024 internship at a bank.';
